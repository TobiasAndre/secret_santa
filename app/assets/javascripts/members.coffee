  ...
  $('body').on 'click', 'a.remove_member', (e) ->
    $.ajax '/members/'+ e.currentTarget.id,
        ...
        success: (data, text, jqXHR) ->
          Materialize.toast('Member removed', 4000, 'green')
          $('#member_' + e.currentTarget.id).remove()
        error: (jqXHR, textStatus, errorThrown) ->
          Materialize.toast('Error removing member', 4000, 'red')
    return false
  ...
  $('.new_member').on 'submit', (e) ->
    $.ajax e.target.action,
        ...
        success: (data, text, jqXHR) ->
          ...
          $('#member_name').focus()
          Materialize.toast('Member added', 4000, 'green')
        error: (jqXHR, textStatus, errorThrown) ->
          Materialize.toast('Error including member', 4000, 'red')
    return false
